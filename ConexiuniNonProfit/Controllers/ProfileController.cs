...
namespace ConexiuniNonProfit.Controllers
{
	public class ProfilesController : Controller
	{
		...
		public IActionResult Show(int id)
		{
			...
			ApplicationUser u1 = db.Users.Where(p => p.Profile == user1).First();
			ApplicationUser u2 = db.Users.Where(p => p.Profile == profile).First();
		...
		public IActionResult Match(int id, int currentIndex = 0)
		{
			...
			var profiles = db.Posts
							 .Where(p => p.ActiuniId == id)
							 .Select(p => p.User.Profile)
							 .Where(profile => profile.UserId != currentUserId)  // Exclude current user's profile
							 .Distinct()
							 .ToList();

			if (currentIndex >= profiles.Count)
			{
				ViewBag.NoMoreUsers = true;
				ViewBag.ActiuniId = id;
				return View(null);
			}
		...
		public IActionResult AddFriend(string id)
		{
			if (id != _userManager.GetUserId(User))
			{
				...
				var user1 = db.Profiles
					.Where(p => p.UserId == _userManager.GetUserId(User)).First();
				ApplicationUser u1 = db.Users.Where(p => p.Profile == user1).First();
				ApplicationUser u2 = db.Users.Where(p => p.Profile == user2).First();
		...
		[Authorize(Roles = "User,Admin")]
		public IActionResult NextUser(int ActiuniId, int currentIndex)
		{
			string currentUserId = _userManager.GetUserId(User);
			...
			{
				ViewBag.NoMoreUsers = true;
				ViewBag.ActiuniId = ActiuniId;
				return View("Match", null);
			}
